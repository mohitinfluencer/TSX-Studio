...
program
    ...
    .action(async (projectId, options) => {
        ...
        try {
            ...
            const { code, name, resolution, fps, durationInFrames } = response.data;
        ...
        export const RemotionRoot: React.FC = () => {
            return (
                <Composition
                    ...
                    fps={${fps || 30}}
                    width={${resolution === '1080p' ? 1080 : 1920}}
                    height={${resolution === '1080p' ? 1920 : 1080}}
                />
            );
        };
...
program
  .command('transcribe')
  .description('Transcribe a local audio/video file (Fast Whisper)')
  .argument('<filePath>', 'Path to media file')
  .action(async (filePath) => {
    const spinner = ora('Initializing transcription engine...').start();
    
    // In a real local setup, this would spawn a python whisper process or equivalent
    spinner.text = 'Running local transcription...';
    
    // Simulate local compute
    setTimeout(() => {
        spinner.succeed(chalk.green('✔ Transcription complete. Result saved to transcript.json'));
        console.log(chalk.dim('\nTip: Run "tsx-studio sync" to upload results to cloud.'));
    }, 3000);
  });
