...
function Card({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      data-slot="card"
      className={cn(
        "bg-white/[0.02] text-card-foreground flex flex-col gap-6 rounded-[32px] border border-white/5 py-6 shadow-2xl backdrop-blur-xl relative overflow-hidden",
...
function CardTitle({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn("leading-none font-black italic uppercase tracking-tighter text-xl", className)}
      {...props}
    />
  )
}
...
function CardDescription({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn("text-muted-foreground text-sm font-medium italic", className)}
      {...props}
    />
  )
}
