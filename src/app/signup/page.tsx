...
export default function SignupPage() {
    ...
    const handleSignup = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!email || !name) {
            toast.error("Cluster registration requires full identity metadata");
            return;
        }

        setIsLoading(true);
        try {
            // For dev mode, we just redirect to dashboard after "simulating" signup
            ...
            toast.success("Node allocated. Welcome to the cluster.");
        } catch (error) {
            toast.error("Registration failed. Cluster rejection.");
            setIsLoading(false);
        }
    };
                    ...
                    <CardContent className="space-y-6 px-8">
                        <form onSubmit={handleSignup} className="space-y-4">
                            <div className="space-y-3">
                                <Label htmlFor="name" className="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1">Alias / Name</Label>
                                <Input
                                    id="name"
                                    value={name}
                                    onChange={(e) => setName(e.target.value)}
                                    placeholder="DESIGN_LEADER_01"
                                    className="h-14 bg-white/5 border-white/5 rounded-2xl px-6 font-medium text-white focus:border-neon-lime/30 focus:ring-neon-lime/20 transition-all placeholder:text-muted-foreground/30 placeholder:uppercase placeholder:text-[10px] placeholder:tracking-widest"
                                />
                            </div>
                            <div className="space-y-3">
                            ...
                            <Button
                                type="submit"
                                disabled={isLoading}
                                className="w-full h-14 text-lg font-black italic bg-white text-black hover:bg-neon-lime hover:shadow-[0_0_30px_rgba(183,255,60,0.4)] rounded-2xl transition-all group"
                            >
                                {isLoading ? "Allocating..." : "Register Cluster"}
                        ...
                        <div className="relative">
                            ...
                            <div className="relative flex justify-center text-[8px] uppercase font-black tracking-[0.4em]">
                                <span className="bg-transparent px-4 text-muted-foreground/40">Secure Auth Bridge</span>
                            </div>
                        </div>
